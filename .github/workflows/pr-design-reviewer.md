...
Review design choices that create a concrete problem for supported behavior, ownership, compatibility, or new public contracts, or impose material runtime cost or substantial maintenance burden.

### Contracts and ownership

- Trace public APIs, trait and FFI contracts, their actual consumers, and the uses promised by new public contracts. A new contract can have a concrete deficiency before it has existing callers. Report incompatible changes or ownership boundaries that prevent correct use, not a preference for a different interface shape.
